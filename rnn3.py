...
import numpy as np
import sys
from layer3 import LstmLayer, OutputLayer
...
class Model:
    def __init__(self, word_dim, hidden_size=[100]):
        ...
        for hdim in self.layer_size:
            ig_params = self.init_params(d1, hdim)
            iv_params = self.init_params(d1, hdim)
            fg_params = self.init_params(d1, hdim)
            og_params = self.init_params(d1, hdim)
            layer = LstmLayer(ig_params, iv_params, fg_params, og_params, hdim) 
            d1 = hdim
    ...
    def init_params(self, d1, hdim):
        params = {}
        params['U'] = np.random.uniform(-np.sqrt(1. / d1), np.sqrt(1. / d1), (hdim, d1))
        params['W'] = np.random.uniform(-np.sqrt(1. / hdim), np.sqrt(1. / hdim), (hdim, hdim))
        params['B'] = np.random.uniform(-np.sqrt(1. / hdim), np.sqrt(1. / hdim), (hdim, ))
        return params
    ...
    def train(self, X, Y, learning_rate=0.005, nepoch=100, evaluate_loss_after=5):
        ...
        for epoch in range(nepoch):
            ...
            for i in range(len(Y)):
                ...
                num_examples_seen += 1
                if i % 20 == 0:
                    print self.layers[0].input_gate.U[:3, :3], '\n'
        return losses
